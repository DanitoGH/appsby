...
# Creating Auth
Like everything in Appsby, authentication is easy to build. You can have multiple authenticators, different user documents, and different parameters for each one.

<TutorialsIntro fileType="authenticator" folder="login" clientSnippet="new AppsbyAuthConnection('login')" initializeName="this.auth"/>


### Logging in
In our client, we might have an input field for an email address, and an input field for a password. We can have anything really - what parameters you base your authentication on are completely up to you.
We'll probably also have a button, that says **Log In**, so that our user can log in.

In order to authenticate the user using the ```AppsbyAuthConnection``` we previously set-up as ```this.auth```, when our login button is clicked we do the following:
```js
this.auth.login({connectionParameters});
```

As stated before, our connection parameters can be anything, but in this case, we're just going to send our user's email and password as connection parameters. Imagining that we're saving the user's email and password inside ```this.state```, we do this as:
```js
this.auth.login({email: this.state.email, password: this.state.password});
```
This will send the connection parameters to the server (along with some internal Appsby data for tracking users and maintaining your app's security). We can then check if a user can login on the server.

## Open ```app-pages/edit/auth.js```
