...
async function ProcessSearch(SearchInterpreter, query, headers, count, cursor, cursorGoesBackward, connectionParameters, deviceId) {

  const view = await new SearchInterpreter(connectionParameters);
  view.props = {};
  view.state = {}
  view.props.connectionParameters = connectionParameters;
  view.props.query = query;
  view.props.count = count;
  view.props.cursor = cursor;
  view.props.cursorGoesBackward = cursorGoesBackward;
...
async function retrieveSearch(searchModule) {
  ...
  let documents = [];
  const items = searchModule.documents.documents;
