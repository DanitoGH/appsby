import {AppsbyDeleteSingleDocument, AppsbyGetSingleDocument,} from "../helpers/fauna-functions";
...
async function ProcessS3(ViewInterpreter, key, appsbyuserid, appsbyusertype, fileName, fileExtension, fileMIME, callback) {
    ...
    const view = await new ViewInterpreter.default(doc.document.connectionParameters);
    ...
    const identifiedUser = await AppsbyGetSingleDocument(appsbyusertype, appsbyuserid)
...
export async function S3Callback(event) {
    ...
    records.forEach((record) => {
        ...
        if (!userId || !userType || !fileName || !fileExtension || !fileMIME || !endpoint || !callback) {
            return AppsbyDeleteSingleDocument("appsbyFileTransitCache", { fileId: key });
        }
